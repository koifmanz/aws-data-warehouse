...
songplay_table_insert = ("""
    ...
    SELECT 
        (events.start_time, events.user_id, events.level, songs.song_id, songs.artist_id, events.session_id,
         events.location, events.user_agent)
    FROM 
        staging_events AS events
    LEFT JOIN 
        staging_songs AS songs ON events.song = songs.title AND events.artist = songs.artist_name
    WHERE 
...
time_table_insert = ("""
    ...
    SELECT 
        (DISTINCT start_time, EXTRACT(hour from start_time), EXTRACT(day from start_time), EXTRACT(week from start_time), 
